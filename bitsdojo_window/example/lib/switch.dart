...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(debugShowCheckedModeBanner: false, home: Scaffold(body: AppBody()));
  }
}
...
class RightSideTopArea extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return WindowTitleBarBox(child: Row(children: [Expanded(child: MoveWindow()), const WindowButtons()]));
  }
}
...
class WindowButtons extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final closeButtonColors = WindowButtonColors(
        mouseOver: Colors.red[700], mouseDown: Colors.red[900], iconNormal: colors.icon, iconMouseOver: Colors.white);
...
class RoundedFlatButton extends StatelessWidget {
  ...
  const RoundedFlatButton({Key? key, this.onPressed, this.color, this.mouseOverColor, this.textColor, this.text})
      : super(key: key);
    ...
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
          shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(12.0), side: const BorderSide(color: Colors.white)),
          backgroundColor: color ?? Colors.grey[900],
          padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 20),
          textStyle: TextStyle(color: textColor ?? Colors.white, fontSize: 12.0, fontWeight: FontWeight.w600)),
      onPressed: onPressed,
    );
  }
}
